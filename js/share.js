var main = function() {
	...
	$('a.redditShare').click(function(e){
		e.preventDefault();
		var loc = $(this).attr('href');
		var title = escape($(this).attr('page-title'));
		window.open('http://reddit.com/submit?url=' + loc + '&title=' + title, 'newwindow', 'height=450, width=550, top='+($(window).height()/2 - 225) +', left='+$(window).width()/2 +', toolbar=0, location=0, menubar=0, directories=0, scrollbars=0');
	});
